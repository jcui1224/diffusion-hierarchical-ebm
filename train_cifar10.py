...
def build_EBM(z_list, args, local_rank, logging):
    from ebm_net import _CIFAR10_netE

    ebm_list = []
    for i, z in enumerate(z_list):
        shape = z.shape
        logging.info(f'z {i} shape: {shape}')
        netEzi = _CIFAR10_netE(shape, args).cuda(local_rank)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint', default='CIFAR10_NVAE_QUAN_DOWNLOADED')
    ...
    parser.add_argument("--iter_metric", type=int, default=5)
    ...
    from config import a100_cifar10 as base_config
    data_base_dir = base_config['data_base_dir']
    args.data = data_base_dir + 'cifar10'
