...
from nvae.utils import one_hot
...
class Normal:
    ...
    def log_p(self, samples, get_sigma=False):
        normalized_samples = (samples - self.mu) / self.sigma
        log_p = - 0.5 * normalized_samples * normalized_samples - 0.5 * np.log(2 * np.pi) - torch.log(self.sigma)
        if get_sigma:
            return log_p, - 0.5 * normalized_samples * normalized_samples, torch.log(self.sigma)
        else:
            return log_p
