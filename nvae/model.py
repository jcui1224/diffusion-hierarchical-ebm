...
from nvae.utils import get_stride_for_cell_type, get_input_size, groups_per_scale
from nvae.distributions import Normal, DiscMixLogistic, NormalDecoder
from nvae.thirdparty.inplaced_sync_batchnorm import SyncBatchNormSwish
...
class AutoEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        log_p_conv, p_rec, n_const = dist.log_p(z, get_sigma=True)
        all_p = [dist]
        ...
        p_reconstruction, log_sigma = 0., 0.
        for p_rec, n_const in zip(all_p_rec, all_n_const):
            p_reconstruction += torch.sum(p_rec, dim=[1, 2, 3])
            log_sigma += torch.sum(n_const, dim=[1, 2, 3])

        return logits, log_q, log_p, kl_all, kl_diag, [p_reconstruction, log_sigma]
    ...
    def get_posterior_jcui7(self, x, test=False):
        posterior = []
        s = self.stem(2 * x - 1.0)
        ...
        for cell in self.dec_tower:
            if cell.cell_type == 'combiner_dec':
                if idx_dec > 0:
                    ...
                    dist = Normal(mu_p + mu_q, log_sig_p + log_sig_q) if self.res_dist else Normal(mu_q, log_sig_q)
                    z, _ = dist.sample(test=test)
                    # z = mu_p + mu_q if self.res_dist else mu_q
                    # apply NF
                    for n in range(self.num_flows):
    ...
    def sample(self, num_samples, t, eps_list=None):
        ...
        dist = Normal(torch.zeros(z0_size).cuda(), torch.zeros(z0_size).cuda(), temp=t)
        if eps_list is None:
            z, _ = dist.sample()
        ...
        for cell in self.dec_tower:
            if cell.cell_type == 'combiner_dec':
                if idx_dec > 0:
                    ...
                    dist = Normal(mu, log_sigma, temp=t)
                    if eps_list is None:
                        z, _ = dist.sample()
                    else:
                        z = dist.sample_given_eps(eps_list[eps_idx])
                    ...
                    z_list.append(z)
                    log_p_list.append(dist.log_p(z))
                # print(f"eps_idx={20-(eps_idx-1)}, s=cell(s, z)")
                # 'combiner_dec'
                s = cell(s, z)
                idx_dec += 1
    ...
    def new_sample_z_by_eps(self, num_samples, t=1.0, eps_list=None):
        scale_ind = 0
    ...
    def new_sample_x_by_s(self, s):
        logits = self.image_conditional(s)
        output = self.decoder_output(logits)
        output_img = output.sample().detach()
        # output_img = output_img.clamp(min=-1., max=1.)
        # output_img = (output_img + 1.) / 2.
        # syn = VAE.decoder_output(logits).sample().detach().clamp(min=0., max=1.)
        return output_img

    def new_sample_eps_by_z(self, posterior):
        scale_ind = 0
        t = 1.0
        eps_idx = 0
        eps_list = []
        num_samples = posterior[0].shape[0]
        z0_size = [num_samples] + self.z0_size
        dist = Normal(torch.zeros(z0_size).cuda(), torch.zeros(z0_size).cuda(), temp=t)
        z = posterior[eps_idx]
        eps_list.append(dist.sample_eps(z).detach())
        eps_idx += 1
